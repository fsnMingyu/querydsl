...
//        List<Member> content = results.getResults(); //data;
    ...
    public void join_on_filtering() {
        List<Tuple> result = jpaQueryFactory
                .select(member, team)
                .from(member)
//                .join(member.team, team) // 내부 조인(inner join)으로 member3, member4 제외됨. //leftJoin 으로 할시 전체 조회.
//                .where(team.name.eq("teamA"))
    ...
    public void fetchJoinNo() {
        ...
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam()); //loading 된 entity인지 확인 가능한 유닛.
